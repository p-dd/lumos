...
using namespace winternl;

typedef LONG NTSTATUS;

#define STATUS_SUCCESS              ((winternl::NTSTATUS) 0x00000000)
#define STATUS_INFO_LENGTH_MISMATCH ((winternl::NTSTATUS) 0xC0000004)
...
struct SYSTEM_PROCESS
{
	...
	LARGE_INTEGER  UserTime;
	LARGE_INTEGER  KernelTime;
	winternl::UNICODE_STRING ImageName;
	LONG           BasePriority;
...
typedef winternl::NTSTATUS(WINAPI* t_NtQueryInfo)(SYSTEM_INFORMATION_CLASS, PVOID, ULONG, PULONG);
...
public:
	...
	DWORD Capture()
	{
		...
		while (true)
		{
			...
			ULONG u32_Needed = 0;
			winternl::NTSTATUS s32_Status = mf_NtQueryInfo(SystemProcessInformation, mp_Data, mu32_DataSize, &u32_Needed);
	...
	DWORD IsThreadWaitingUserRequest(SYSTEM_THREAD* pk_Thread, BOOL* pb_Suspended)
	{
		if (!pk_Thread)
			return ERROR_INVALID_PARAMETER;

		*pb_Suspended = (pk_Thread->ThreadState == Waiting &&
			pk_Thread->WaitReason == WrLpcReply);
		return 0;
	}

	DWORD IsThreadWaitingUserRequest(DWORD u32_PID, DWORD u32_TID, BOOL* pb_Suspended)
	{
		DWORD u32_Error = Capture();
		if (u32_Error)
		{
		...
		SYSTEM_PROCESS* pk_Proc = FindProcessByPid(u32_PID);
		if (!pk_Proc)
		{
		...
		SYSTEM_THREAD* pk_Thread = FindThreadByTid(pk_Proc, u32_TID);
		if (!pk_Thread)
		{
		...
		return IsThreadWaitingUserRequest(pk_Thread, pb_Suspended);
	}
...
#undef STRING_WININTERNL
#undef UNICODE_STRING_WININTERNL
